            ...
            TextureAtlas.prototype.load = function (atlasText, textureLoader, callback) {
                ...
                var iterateParser = function () {
                    while (true) {
                        ...
                        if (line.length == 0)
                            page = null;
                        else if (/^[a-z]{2}_[a-z]{2}$/.test(line) || line == "locale:") {
                            continue;
                        }
                        else if (/\.atlas$/.test(line)){
                            baseUrl = line.substr(0, line.lastIndexOf("/")) + "/";
                        }
                        else if (!page) {
                            page = new TextureAtlasPage();
                            page.name = line;
                            ...
                            textureLoader(baseUrl + line, function (texture) {
...
(function (pixi_spine) {
    ...
    function atlasParser() {
        return function atlasParser(resource, next) {
            ...
            if (resource.metadata && resource.metadata.atlasRawData) {
                ...
                var TextureAtlasReader = pixi_spine.core.TextureAtlasReader;
                var _this = this;
                this.add(resource.name + '_atlas', atlasPath, atlasOptions, function (atlasResource) {
                    //createSkeletonWithRawAtlas(atlasResource.xhr.responseText);

                    var list = [];
                    var reader = new TextureAtlasReader(atlasResource.xhr.responseText);
                    while(true){
                        var line = reader.readLine();
                        if (line == null) break;
                        ...
                        if (line == "locale:") {
                            while(true) {
                                ...
                                line = line.trim();
                                if (/^[a-z]{2}_[a-z]{2}$/.test(line) && line == lan.toLowerCase()) {
                                    while(true) {
                                        line = reader.readLine();
                                        if (line == null || /^[a-z]{2}_[a-z]{2}$/.test(line.trim())) break;
                            ...
                            if (/\.atlas$/.test(line)) {
                                line = line.replace("[lan]", lan.toLowerCase());
                                list.push(line);
                            }
                        }
                    }

                    if(list.length == 0) return createSkeletonWithRawAtlas(atlasResource.xhr.responseText);

                    var count = 0, textList = [atlasResource.xhr.responseText];
                    var done = function(text, path){
                        textList.push("\r\n" + path + "\r\n" + text);
                        count ++;

                        if(count == list.length){
                            createSkeletonWithRawAtlas(textList.join(""));
                        }
                    };

                    var load = function(name, path){
                        var resource = _this.resources[name];
                        if(resource){
                            if(resource.data){
                                done(resource.data, path);
                            }else{
                                resource.onAfterMiddleware.once(function(){
                                    done(resource.data, path);
                                });
                            }
                        }else{
                            _this.add(name, baseUrl + path, atlasOptions, function(resource){
                                done(resource.xhr.responseText, path);
                            })
                        }
                    };
                    ...
                    for(var i = 0; i < list.length; i++){
                        var path = list[i];
                        var name = path.substring(path.lastIndexOf("/") + 1, path.lastIndexOf(".")) + "_sub_atlas";
                        load(name, path);
                    }
                });
            }
    ...
    function imageLoaderAdapter(loader, namePrefix, baseUrl, imageOptions) {
        ...
        return function (line, callback) {
            ...
            var name = "sub_" + line.substr(line.lastIndexOf("/") + 1);
